...
function multiply(a, b) { //eslint-disable-line
  let myProduct = a * b;
  let string = `The product of ${a} and ${b} is ${myProduct}.`;
...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  myProduct = multiply (myProduct,c)[0];

  let sumStr = `${a} and ${b} and ${c} sum to ${mySum}.`;
  let sumProduct = `The product of ${a} and ${b} and ${c} is ${myProduct}.`;

  return [mySum, myProduct, sumStr, sumProduct];
}
...
function sumArray(sumArr) { //eslint-disable-line
  let mySum = sum(sumArr[0],sumArr[1])[0];
  mySum = sum(mySum,sumArr[2])[0];

  let myStr = `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${mySum} is their sum.`;
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  for(i=0;i<dynamicArray.length;i++){
    myProduct = multiply(myProduct,dynamicArray[i])[0];
    if(i===dynamicArray.length-1){
      myStr = `${myStr}${dynamicArray[i]}`;
    } else {
      myStr = `${myStr}${dynamicArray[i]},`;
    }
  }

  myStr = `The numbers ${myStr} have a product of ${myProduct}.`;
